...
export default function calculateAverages(room) {
	...
	beerList.forEach(beer => {
		...
		db.collection('beers').doc(beer.beerID).get().then(doc => {
			let beerDoc = doc.data();
			beerDoc.avgAppearanceScore = (avgAppearanceScore() + beerDoc.avgAppearanceScore) / 2;
			beerDoc.avgScore = (avgScore() + beerDoc.avgScore) / 2;
			beerDoc.avgSensationsScore = (avgSensationsScore() + beerDoc.avgSensationsScore) / 2;
			beerDoc.avgSmellScore = (avgSmellScore() + beerDoc.avgSmellScore) / 2;
			beerDoc.avgSubjectiveScore = (avgSubjectiveScore() + beerDoc.avgSubjectiveScore) / 2;
	...
	participants.forEach(participant => {
		if(!participant.isEliminated) {
			...
			let appearance = () => {
				let sum = 0;
				beerList.forEach(beer => sum += beer.userScores[beer.userScores.indexOf(beer.userScores.find(scores => scores.userID == participant.userID))].appearanceScore);
				return sum/beerList.length;
			}

			let overall = () => {
				let sum = 0;
				beerList.forEach(beer => sum += beer.userScores[beer.userScores.indexOf(beer.userScores.find(scores => scores.userID == participant.userID))].avgScore);
				return sum/beerList.length;
			}

			let sensations = () => {
				let sum = 0;
				beerList.forEach(beer => sum += beer.userScores[beer.userScores.indexOf(beer.userScores.find(scores => scores.userID == participant.userID))].sensationsScore);
				return sum/beerList.length;
			}

			let smell = () => {
				let sum = 0;
				beerList.forEach(beer => sum += beer.userScores[beer.userScores.indexOf(beer.userScores.find(scores => scores.userID == participant.userID))].smellScore);
				return sum/beerList.length;
			}

			let subjective = () => {
				let sum = 0;
				beerList.forEach(beer => sum += beer.userScores[beer.userScores.indexOf(beer.userScores.find(scores => scores.userID == participant.userID))].subjectiveScore);
				return sum/beerList.length;
			}

			let taste = () => {
				let sum = 0;
				beerList.forEach(beer => sum += beer.userScores[beer.userScores.indexOf(beer.userScores.find(scores => scores.userID == participant.userID))].tasteScore);
				return sum/beerList.length;
			}
	...
	db.collection('rooms').doc(room.id).update({participants, beerList, inProgress: false}).then(() => {
